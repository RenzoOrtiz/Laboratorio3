        ...
        if (event.path === "/envios/pendientes") {
            ...
            await docClient.scan(params, (err, data) => {
                if (!err) {
                    resp = {
                        statusCode: 200,
                        body: (data.Items.length) ? data : 'El listado de envios pendientes está vacio',
                    }
                }
                else {
                    resp = {
                        statusCode: 500,
                        body: 'El listado de envios pendientes no está disponible',
                    }
                }
            }).promise();
    ...
    if (event.httpMethod = "POST"){
        ...
        if (event.path === '/envios') {
            ...
            if (!event.body || !body.email || !body.destino) {
                ...
                return {
                    statusCode: 400,
                    body: 'La informacion enviada está incompleta'
                }
            }
            let params = {
                TableName: 'Envio',
